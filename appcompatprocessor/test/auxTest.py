...
class WeightedRandomizer:
    def __init__ (self, weights):
        self.__max = .0
        self.__weights = []
        for value, weight in list(weights.items ()):
            self.__max += weight
...
class ACPProvider(BaseProvider):
    ...
    def path(self):
        ...
        # Build path
        current_subpath = ''
        for i in range(1, folderNumWeightedRandomizer.random()):
...
def build_fake_DB(hosts = 10, seed = random.randint(0,10000), database_file = None):
    ...
    with appDB.DBClass(database_file, "False", settings.__version__) as DB:
        ...
        for i in range(0,hosts):
            ...
            for i in range(1, random.randint(400,800)):
        ...
        list_FilePath_ID = [(v, k) for k, v in list(filePaths_dict.items())]

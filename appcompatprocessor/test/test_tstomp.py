...
class TestAppTstomp(TestCase):
    ...
    def test_Tstomp1(self):
        ...
        with appDB.DBClass(self.testset1, settings.__version__) as DB:
            ...
            num_hits = len(ret)
            self.assertEqual(num_hits, 4, "test_Tstomp1 failed!")
            self.assertEqual(ret[2][1][5], 'kernel32.dll', "test_Tstomp1 failed!")
            self.assertEqual(ret[3][1][5], 'badboy.exe', "test_Tstomp1 failed!")
            self.assertEqual(ret[2][1][2], ret[3][1][2], "test_Tstomp1 failed!")
    ...
    def test_Tstomp2(self):
        ...
        with appDB.DBClass(self.testset1, settings.__version__) as DB:
            ...
            num_hits = len(ret)
            self.assertEqual(num_hits, 6, sys._getframe().f_code.co_name)
            self.assertEqual(ret[5][1][5], 'badboy1.exe', "test_Tstomp2 failed!")

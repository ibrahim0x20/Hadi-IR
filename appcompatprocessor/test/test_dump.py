                            ...
                            if item == 'Last Modified,Last Update,Path,File Size,Exec Flag':
                                ...
                                if m:
                                    if m.group(1) == '0001-01-01 00:00:00':
                                        ...
                                        LastModified = datetime.datetime.strptime(str(m.group(1)),
                                        '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S')
                                    if m.group(2) == '0001-01-01 00:00:00':
                                        LastUpdate = 'N/A'
                                    else:
                                        LastUpdate = datetime.datetime.strptime(str(m.group(2)),
                                        '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d %H:%M:%S')
                                    file.write("%s,%s,%s\\%s,%s,%s\r\n" % (
                                    LastModified, LastUpdate, str(m.group(3)), str(m.group(4)), str(m.group(5)),
                                    str(m.group(6))))

    ...
    def write_to_DB(self, number, timer):
        with closing(self.conn.cursor()) as c:
            try:
                ...
                self.logger.error("SQLITE error: %s" % (str(er)))
                raise
            time.sleep(timer)
            self.conn.commit()
...
class TestAppMPEngine(TestCase):
    ...
    def test_MPEngine_DatabaseLocked(self):
        try:
            ...
            # Add tasks
            task_list = [i for i in range(1, num_tasks + 1)]
            ...
            self.assertEqual(len(results), num_tasks, "test_MPEngine_end2end")
            self.assertEqual(results[-1], num_tasks, "test_MPEngine_end2end")
        ...
        # Pass
        self.assertEqual(1, 1, "test_MPEngine_end2end")
    ...
    def test_MPEngine_ProducerSimple(self):
        ...
        try:
            ...
            # Add tasks
            task_list = [i for i in range(1, 5)]
            mpe.addTaskList(task_list)
        ...
        # Pass
        self.assertEqual(1, 1, "test_MPEngine")
    ...
    def test_MPEngine_brute_producers(self):
        try:
            ...
            for i in range(0,99): mpe.addProducer()
            for i in range(0,99): mpe.removeProducer()
    ...
    def test_MPEngine_brute_both(self):
        try:
            ...
            for i in range(0, max_procs): mpe.addProducer()
            for i in range(0, max_procs): mpe.removeProducer()
            for i in range(0, max_procs): mpe.addConsumer()
            for i in range(0, max_procs): mpe.removeConsumer()
    ...
    def test_MPEngine_regressionBug(self):
        try:
            ...
            mpe = MPEngineProdCons(maxCores, WkrTestProd, WkrTestCons)
            # Add tasks
            task_list = [i for i in range(1, 8)]
            mpe.addTaskList(task_list)
    ...
    def test_MPEngine_restart(self):
        try:
            ...
            with MPEngineProdCons(6, WkrTestProd, WkrTestCons) as mpe:
                ...
                mpe.addProducer(["3"])
                mpe.addProducer(["4"])
                self.assertEqual(mpe.getProducerCount(), 4, "test_MPEngine")
                time.sleep(1)
                mpe.restartProducers()
                self.assertEqual(mpe.getProducerCount(), 4, "test_MPEngine")

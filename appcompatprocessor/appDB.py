    ...
    def appConnectDB(self, dbfilenameFullPath=None):
        ...
        if os.path.isfile(self.dbfilenameFullPath):
            ...
            try:
                ...
                logger.exception("Error loading DB %s [%s]" % (self.dbfilenameFullPath, str(er)))
                exit()
        ...
        if os.path.isfile(self.dbfilenameFullPath):
            ...
            with closing(tmpconn.cursor()) as c:
                try:
                    ...
                    if self.versionCode > self.versionDB[0]:
    ...
    def QuerySpinner(self, query):
        ...
        with closing(tmpconn.cursor()) as c:
            try:
                ...
                logger.exception("SQLITE error: %s [%s]" % (str(er), query))
                raise
            else:
                return data

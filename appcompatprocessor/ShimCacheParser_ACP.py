...
constants = {
    ...
    # Windows 6.1 (Win7 and Server 2008 R2)
    "CACHE_MAGIC_NT6_1": 0xbadc0fee,
    "CACHE_HEADER_SIZE_NT6_1": 0x80,
    "NT6_1_ENTRY_SIZE32": 0x20,
    "NT6_1_ENTRY_SIZE64": 0x30,
...
def read_cache(cachebin, quiet=False):
    ...
    try:
        ...
        #     if magic == value:
        #         print("magic in hex = 0x%x" % magic)
        #         print(f"magic in Dec = {magic}")
        #         print(f"{name}:")
        #         print(f"  Hex:  {hex(value)}")
        #         print(f"  Dec:  {value}")
        # magic_bytes = struct.pack("<L", magic)  # Convert integer to little-endian bytes
        # print(f"magic in byte = {repr(magic_bytes)}")
        # for name, value in byte_constants.items():
        #     if magic_bytes == value:
        #
        #         print(f"magic in byte = {magic_bytes}")
        #         print("magic in hex = 0x%x" % magic)
        #         print(f"magic in Dec = {magic}")
        #         print(f"{name}:")
        #         print(f"  Bytes: {value}")
        ...
        #     # print(cachebin[WIN10_CREATORS_STATS_SIZE:WIN10_CREATORS_STATS_SIZE+4] )
        #     print("*****************************************************************")
        #     print(f"CACHE_MAGIC_NT6_1 = {CACHE_MAGIC_NT6_1}")
        #     print("*****************************************************************")
        # print(f"Extracted magic: {hex(magic)} (Expected: {hex(CACHE_MAGIC_NT6_1)})")
        ...
        # Windows 10 will use a different magic dword, check for it
            ...
            # print("*****************************************************************")
            # print("\n\nFrom WIN10_STATS_SIZE")
            # print(cachebin[WIN10_STATS_SIZE:WIN10_STATS_SIZE+4])
            # print(f"Windows 10 Magic = {WIN10_MAGIC}")
            # print("*****************************************************************")
            if not quiet:
        ...
        # Windows 10 Creators Update will use a different STATS_SIZE, account for it
            ...
            # print("\n\nFrom WIN10_CREATORS_STATS_SIZE")
            # print(cachebin[WIN10_CREATORS_STATS_SIZE:WIN10_CREATORS_STATS_SIZE + 4])
            # print(f"Windows 10 Magic = {WIN10_MAGIC}")
            if not quiet:
...
def read_nt6_entries(bin_data, entry):
    ...
    try:
        ...
        print('[-] Error reading Shim Cache data - In read_mt6_entries: %s...' % err)
        return None
...
def read_from_hive(hive, quiet=False):
    ...
    if reg.root().path() in partial_hive_path:
        if reg.root().path() == 'Session Manager':
            ...
            if list(reg.root().find_key('AppCompatCache').values()):
                print("[+] Partial hive -- 'AppCompatCache' or 'AppCompatibility'")
                keys = list(reg.root().find_key('AppCompatCache').values())
        else:

            ...
            try:
                ...
                if sanityCheckOK:
                    ...
                    if x.FilePath is None:
                        ...
                        if x.FilePath.startswith("b'") or x.FilePath.startswith('b"'):
                            # Remove the b'' or b"" prefix and trailing quote, then replace double backslashes
                            x.FilePath = x.FilePath[3:-1].replace('\\\\', '\\')
                ...
                self.logger.warning("Exception processing row (%s): %s [%s / %s / %s]" % (str(e), x, exc_type, fname, exc_tb.tb_lineno))
...
class appLoadCons(MPEngineWorker):
    ...
    def do_work(self, entries_fields_list):
        ...
        if entries_fields_list:
            numFields = len(list(entries_fields_list[0]._asdict().keys())) - 4
            ...
            try:
                ...
                print(("%s - Sqlite error: %s" % (self.proc_name, str(er))))
                self.logger.debug("%s - Sqlite error: %s" % (self.proc_name, str(er)))
            self.conn.commit()
...
class Task(object):
    ...
    def __call__(self):
        ...
        for item in self.data:
            ...
            # except Exception as e:
            #     logger.error("The error is coming from the Task class initial")
                # logger.error("Task - Error processing: %s (%s)" % (file_fullpath, str(e)))
            ...
            if file_name_original is None:
                if ingest_plugins[ingest_type].matchFileNameFilter(file_name_fullpath):
                    ...
                    try:
                        ...
                        logger.exception("GetIDForHosts - Error processing: %s (%s)" % (file_name_fullpath, str(e)))
            else:
...
def appLoadMP(pathToLoad, dbfilenameFullPath, maxCores, governorOffFlag):
    ...
    file_filter = '|'.join([v.getFileNameFilter() for k,v in ingest_plugins.items()])
    # Add zip extension
    file_filter += "|.*\.zip$"
